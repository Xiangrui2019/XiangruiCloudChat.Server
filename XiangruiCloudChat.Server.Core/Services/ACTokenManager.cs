    ...
    public class ACTokenManager
    {
        ...
        public string ValidateAccessToken(string value)
        {
            ...
            try
            {
                ...
                {
                    throw new APIModelException(ErrorType.Timeout, "Token was timed out!");
                }
                if (!_rsa.VerifyData(tokenBase64.Base64ToString(), tokenSign))
                {
                    throw new APIModelException(ErrorType.Unauthorized, "Invalid signature! Token could not be authorized!");
                }
            }
            catch
            {
                throw new APIModelException(ErrorType.Unauthorized, "Token was not in a valid format and can not be verified!");
            }
            return token.AppId;
        }
    }
